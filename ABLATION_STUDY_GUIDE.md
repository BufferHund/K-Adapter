...
我们将 `layers=1` 和 `layers=4` 的实验结果，与实验一中 `size=64`（即 `layers=2`）的基线结果进行对比。

| Adapter 内部层数 | Micro F1 (测试集) | Macro F1 (测试集) |
| :--- | :--- | :--- |
| 1 | **0.689** | 0.567 |
| 2 (基线) | 0.686 | 0.563 |
| 4 | 0.679 | **0.569** |
...
与之前的尺寸消融研究结论高度一致：**Adapter的性能对内部复杂度变化同样不敏感，甚至更简单的模型表现更好。**

#### 详细解读

1.  **性能惊人地稳定**: 从1层、2层到4层，Adapter的综合性能指标 Micro F1 分数基本都稳定在 **0.68** 左右，几乎没有变化。这说明对于此任务，增加Adapter的内部深度并不能带来有效的性能收益。

2.  **简单即是胜利 (Simpler is Better)**: 值得注意的是，层数最少的 (`layers=1`) Adapter 取得了最高的 Micro F1 分数 (0.689)。这有力地暗示，对于Adapter这种“小模型”，过度的内部复杂度不仅无益，反而可能因为引入了更多的参数和更难的优化问题，对性能产生微小的负面影响。

3.  **默认设置的合理性**: 实验也证明，原始实现中 `layers=2` 的默认设置是一个非常合理且高效的基线选择。

#### 结论

对于在OpenEntity任务上使用Factual Adapter，**没有必要增加其内部Transformer的层数**。使用更简单的1层或默认的2层结构，可以在获得近乎最佳性能的同时，保持模型的简洁和高效率。

---

## 5. 实验四 (已完成)：与全量微调的对比分析
...
- **目标**: 量化对比不同Adapter配置下模型的推理延迟、吞吐量和硬件（GPU显存）占用。
- **方法**: 使用 `benchmark_inference.py` 脚本，测试原始 RoBERTa-large 模型、加载 Factual Adapter 的模型、以及同时加载 Factual 和 Linguistic Adapter 的模型在不同 Batch Size 下的性能。

##### 4.2.1 实验结果

**1. 基线: 原始 RoBERTa-large**

| Batch Size | Average Latency (ms) | Throughput (samples/sec) | Peak GPU Memory (MB) |
| :--------- | :------------------- | :----------------------- | :------------------- |
| 1          | 26.676               | 37.49                    | 1392.22              |
| 8          | 111.611              | 71.68                    | 1535.75              |
| 16         | 223.866              | 71.47                    | 1699.77              |
| 32         | 421.133              | 75.99                    | 2027.83              |

**2. 实验组1: RoBERTa + Factual Adapter**

| Batch Size | Average Latency (ms) | Throughput (samples/sec) | Peak GPU Memory (MB) |
| :--------- | :------------------- | :----------------------- | :------------------- |
| 1          | 36.058               | 27.73                    | 1584.25              |
| 8          | 131.591              | 60.79                    | 1731.52              |
| 16         | 259.807              | 61.58                    | 1899.57              |
| 32         | 491.540              | 65.10                    | 2237.67              |

**3. 实验组2: RoBERTa + Factual & Linguistic Adapter**

| Batch Size | Average Latency (ms) | Throughput (samples/sec) | Peak GPU Memory (MB) |
| :--------- | :------------------- | :----------------------- | :------------------- |
| 1          | 43.807               | 22.83                    | 1775.50              |
| 8          | 152.145              | 52.58                    | 1926.15              |
| 16         | 296.917              | 53.89                    | 2098.33              |
| 32         | 563.723              | 56.77                    | 2444.43              |

##### 4.2.2 综合分析与结论

| 模型配置 | Batch Size | Latency (ms) | Throughput (samples/sec) | Peak GPU Memory (MB) |
| :------- | :--------- | :----------- | :----------------------- | :------------------- |
| **Base** | 1          | 26.676       | 37.49                    | 1392.22              |
| **Base** | 32         | 421.133      | 75.99                    | 2027.83              |
| **+Fac** | 1          | 36.058       | 27.73                    | 1584.25              |
| **+Fac** | 32         | 491.540      | 65.10                    | 2237.67              |
| **+Fac+Lin** | 1          | 43.807       | 22.83                    | 1775.50              |
| **+Fac+Lin** | 32         | 563.723      | 56.77                    | 2444.43              |

**核心发现**:

1.  **Adapter 引入推理开销**: 无论是单Adapter还是双Adapter，都会导致单次推理的延迟增加和吞吐量下降。相比Base模型，单Adapter增加了约 35% 的延迟，双Adapter增加了约 64% 的延迟。
2.  **显存占用增加**: Adapter作为额外的模块，会增加模型的显存占用。每个Adapter大约增加200-300MB的显存占用。
3.  **Adapter 数量与开销呈正相关**: 无论是延迟、吞吐量还是显存，增加Adapter的数量都会导致开销的线性增加。

**结论**:

K-Adapter 方案在实现参数高效性的同时，确实会引入一定的推理开销（包括延迟、吞吐量下降和显存占用增加）。这种开销与Adapter的数量呈正相关。在实际部署时，需要根据具体的应用场景（例如，是否对实时性有极高要求，或GPU显存是否非常紧张）来权衡 Adapter 带来的性能提升与其引入的推理开销。

---

## 6. 实验五：预训练数据源对知识迁移能力的影响

- **目标**: 探究使用不同类型的数据集进行Adapter预训练后，其知识迁移到下游任务的能力差异。具体来说，对比在 **大规模通用知识图谱 (T-REx)** 上预训练的Adapter与在 **小规模、任务特定的关系分类数据集 (TACRED)** 上预训练的Adapter，在OpenEntity任务上的性能表现。

- **方法**:
    - **实验组 (TACRED-Adapter)**:
        1.  **预训练**: 使用 `data/tacred` 数据集作为预训练数据，训练一个新的Factual Adapter。
            - **关键修复**: 由于原始 `preprocess_tacred.py` 脚本与数据格式不匹配，我们重写了该脚本，使其能够正确解析原始TACRED数据，进行分词，并将实体从字符级别位置映射到词元级别索引。此外，我们还修复了 `utils_glue.py` 中的数据加载器和处理器，使其能够正确处理TACRED的标签体系。
            - **执行**: 运行 `run_pretrain_on_tacred.sh`。
        2.  **微调**: 加载上一步产出的 `tacred-adapter`，在OpenEntity任务上进行微调。
            - **执行**: 运行 `run_finetune_with_tacred_adapter.sh`。
    - **基线组 (T-REx-Adapter)**: 复用实验一中，使用T-REx预训练、`adapter_size=64` 的实验结果作为基线。

- **执行步骤**:
    1.  运行重写后的 `preprocess_tacred.py` 脚本准备数据。
    2.  运行 `run_pretrain_on_tacred.sh` 训练 `tacred-adapter`。
    3.  运行 `run_finetune_with_tacred_adapter.sh` 在OpenEntity上进行评估。
    4.  从输出结果中收集最终的F1分数，并与基线进行对比。

### 实验五结果与分析

| 预训练数据集 | Adapter 尺寸 | Micro F1 (测试集) | Macro F1 (测试集) |
| :--- | :--- | :--- | :--- |
| T-REx (基线) | 64 | **0.686** | **0.563** |
| TACRED (本实验) | 64 | 0.348 | 0.256 |

#### 核心发现

**预训练数据的选择对Adapter的知识迁移能力有决定性影响。** 使用小规模、任务特定的TACRED数据集预训练的Adapter，其在下游OpenEntity任务上的表现远逊于使用大规模、通用T-REx数据集预训练的Adapter。

#### 详细解读

1.  **通用知识 vs. 特定知识**: T-REx源于维基百科，覆盖了广泛的通用事实知识。在其上预训练的Adapter能够学习到一种普适的、可迁移的知识表示，因此能有效适应新的下游任务。相比之下，TACRED专注于实体间的关系分类，其包含的知识非常狭窄和特异化。
2.  **知识无法迁移**: 在TACRED上训练的Adapter，其内部参数很可能已经过拟合于“判断实体关系”这一特定任务。当面对一个完全不同（但相关）的“判断实体类型”任务时，这些高度特化的参数不仅无法提供帮助，甚至可能因为引入了不相关的归纳偏置（inductive bias）而损害了模型的性能。
3.  **性能断崖式下跌**: Micro F1分数从0.686暴跌至0.348，下降了近50%，这清晰地表明了知识迁移的失败。

#### 结论

实验五的结果强有力地证明了K-Adapter成功的关键之一在于 **使用大规模、高覆盖率的通用知识源进行预训练**。这使得Adapter能够成为一个即插即用的、真正意义上的“知识插件”。如果预训练数据过于领域化或任务特定，Adapter的通用性和可迁移性将大打折扣，使其应用价值严重受限。
